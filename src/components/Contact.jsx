...
export default function ContactSection() {
  ...
  return (
    <section
      id="contact"
      className="relative min-h-screen px-6 py-12 flex flex-col justify-center overflow-hidden"
    >
      {/* Arrière-plan */}
      <div className="absolute inset-0 z-[-1] bg-gradient-to-b from-purple-200 to-blue-100 transition-colors duration-500 pointer-events-none" />
      ...
      {alertVisible && (
        <div
          className={`
            ...
            px-6 py-3 rounded-full shadow-md flex items-center gap-3
            ${
              status === "success"
      ...
      <motion.div
        ...
        className="max-w-4xl w-full mx-auto grid grid-cols-1 md:grid-cols-5 gap-8 md:gap-12"
      >
        {/* Titres & Informations de contact */}
        <div className="flex flex-col md:col-span-2 h-full items-center md:items-start gap-8 md:gap-0">
          {/* Titres */}
          <motion.div
            className="flex flex-col justify-center flex-1 pb-0 mb:pb-4 items-center md:items-start"
            variants={titleVariant}
          >
            <p className="text-xs uppercase tracking-[0.2em] text-purple-600/50 font-semibold mb-2 text-center md:text-left">
              {t("contact.subtitle")}
            </p>
            <h2 className="text-3xl md:text-5xl font-extrabold text-gray-900 leading-tight text-center md:text-left">
              {t("contact.title")}
            </h2>
          </motion.div>

          {/* Infos & Réseaux sociaux */}
          <div className="flex flex-col justify-end gap-8 md:gap-12 pb-0 mb:pb-4 w-full items-center md:items-start">
            {/* Localisation & Email */}
            <motion.div
              variants={infoVariants}
              className="flex flex-col sm:flex-row md:flex-col gap-3 sm:gap-8 md:gap-0 space-y-0 md:space-y-4 text-base w-full justify-center md:justify-start"
            >
              {CONTACT_INFOS.map((info) => (
                <div key={info.type} className="flex items-center justify-center md:justify-start">
                  ...
                  >
                    {info.icon === "FaMapMarkerAlt" && <FaMapMarkerAlt className="text-gray-700/70 group-hover:text-purple-600/50 transition duration-200" size={18} />}
                    {info.icon === "FaEnvelope" && <FaEnvelope className="text-gray-700/70 group-hover:text-purple-600/50 transition duration-200" size={18} />}
                    <span className="text-gray-700/70 font-medium group-hover:text-purple-600/50 transition duration-200">
            ...
            >
              {SOCIAL_LINKS.map((item) => (
                <a
                  ...
                  className="text-gray-700/70 hover:text-purple-600/50 hover:scale-110 transition duration-200"
                >
        ...
        <motion.form
          ...
          className="w-full md:col-span-3 flex flex-col justify-center"
        >
          <div className="w-full rounded-2xl bg-white/10 border-2 border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.10)] backdrop-blur-[35px] px-8 py-12 space-y-12">
            ...
            <div className="space-y-6">
                ...
                <div className="relative">
                  <FiUser
                    className={`absolute left-4 top-1/2 -translate-y-1/2 text-lg pointer-events-none transition-colors duration-200 ${
                      nameValue ? "text-gray-800" : "text-gray-400/70"
                    }`}
                  />
                  <input
                    id="contact-name"
                    ...
                    className="w-full rounded-full pl-10 pr-4 py-2.5 border-2 border-white/15 bg-white/15 focus:outline-none focus:ring-2 focus:ring-purple-500/30 transition text-gray-800 placeholder:text-gray-400/70"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <FiMail
                    className={`absolute left-4 top-1/2 -translate-y-1/2 text-lg pointer-events-none transition-colors duration-200 ${
                      emailValue ? "text-gray-800" : "text-gray-400/70"
                    }`}
                  />
                  <input
                    id="contact-email"
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  className="w-full rounded-3xl px-4 py-3 border-2 border-white/15 bg-white/15 focus:outline-none focus:ring-2 focus:ring-purple-500/30 transition text-gray-800 placeholder:text-gray-400/70 resize-none"
                />
              </div>
            </div>
            ...
            <button
              ...
              className="w-full rounded-full px-4 py-3 bg-purple-600/30 text-white font-semibold shadow-lg hover:bg-purple-600/40 transition duration-200 disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
            >
