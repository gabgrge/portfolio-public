        ...
        <div className="relative group">
          ...
          {modalOpen && modalProject && (
            <div className="fixed inset-0 z-50 flex items-center justify-center">
              ...
              <div
                className="relative z-10 w-full max-w-md sm:max-w-xl mx-4 rounded-2xl shadow-2xl p-6 sm:py-12 sm:px-10 h-[80vh] sm:h-auto sm:min-h-3/5 border border-white/30 bg-white/30 backdrop-blur-lg flex flex-col items-center justify-center animate-fade-in"
                style={{ maxHeight: "90vh" }}
              >
                {/* Bouton de fermeture */}
                <button
                  className="absolute top-2 right-2 p-1 sm:p-2 rounded-full text-gray-700 hover:rotate-90 hover:scale-110 hover:text-gray-800 transition duration-300 focus:outline-none focus:text-blue-700 z-20 cursor-pointer"
                  onClick={() => setModalOpen(false)}
                  aria-label="Fermer"
                  tabIndex={0}
                  type="button"
                ...
                <p className="text-sm text-gray-800/70 mb-3 font-medium self-start text-left">
                ...
                <div className="w-full overflow-y-auto">
                  ...
                  <div className="text-gray-900 text-base leading-relaxed w-full">
                  ...
                  <div className="flex flex-wrap gap-2 mt-4 justify-center">
                    {modalProject.tech?.map((tech, i) => (
                      <span
                        key={i}
                        className="text-blue-700 text-xs font-semibold px-3 py-1 rounded-full border border-blue-700"
                      >
          ...
          <Slider {...settings} ref={sliderRef}>
            {projects.map((project, index) => (
              <div key={index} className="pt-4 pb-8 px-2 sm:px-4">
                <div className="relative min-h-[400px] sm:min-h-[500px] flex flex-col sm:flex-row items-center bg-white/90 rounded-2xl shadow-xl overflow-hidden gap-6 p-6 transition">
                  {/* Flèche gauche (desktop, visible au survol) */}
                  <button
                    type="button"
                    aria-label="Précédent"
                    onClick={() => sliderRef.current?.slickPrev()}
                    className="hidden sm:flex absolute inset-y-0 left-0 w-14 items-center justify-center bg-white/30 backdrop-blur-md transition-opacity duration-200 opacity-0 hover:opacity-100 focus:opacity-100 z-10 rounded-l-2xl border-r border-white/30 cursor-pointer"
                    tabIndex={0}
                  >
                    <FaChevronLeft
                      size={28}
                      className="text-blue-600 transition-transform duration-100 group-active:scale-90"
                    />
                  </button>
                  ...
                  <button
                    ...
                    className="hidden sm:flex absolute inset-y-0 right-0 w-14 items-center justify-center bg-white/30 backdrop-blur-md transition-opacity duration-200 opacity-0 hover:opacity-100 focus:opacity-100 z-10 rounded-r-2xl border-l border-white/30 cursor-pointer"
                    tabIndex={0}
                  >
                    <FaChevronRight
                      size={28}
                  ...
                  <div className="w-full sm:w-1/2 text-center sm:text-left flex flex-col justify-center">
                    ...
                    <div className="flex flex-wrap justify-center sm:justify-start gap-2 mb-6">
                      {project.tech.map((tech, i) => (
                        <span
                          key={i}
                          className="bg-white text-blue-600 text-xs font-medium px-2 py-1 rounded-full"
                        >
                          {tech}
                        </span>
                      ))}
                    ...
                    <div className="flex flex-row gap-3 justify-center sm:justify-start mt-2">
                      ...
                      <button
                        className="px-4 py-2 bg-blue-600 text-white text-sm rounded-full shadow hover:bg-blue-700 active:scale-90 transition duration-150 cursor-pointer"
                        onClick={() => {
                      ...
                      {project.repo && (
                        <a
                          ...
                          className="px-4 py-2 border border-blue-600 text-blue-600 text-sm rounded-full shadow hover:bg-blue-600 hover:text-white active:scale-90 transition duration-150 flex items-center gap-2"
                        >
                          <FiGithub size={18} />
                          GitHub
                        </a>
