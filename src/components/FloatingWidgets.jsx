...
/**
 * FloatingWidgets
 * Affiche les widgets flottants (langue & thème) en bas à droite.
...
export default function FloatingWidgets({
  ...
  useEffect(() => {
    // Sélectionne le conteneur qui scroll (desktop uniquement)
    ...
    function handleScroll() {
      ...
      // Vérifie si le widget est au-dessus de la section projets
      const overlap =
  ...
  return (
    <div
      id="floating-widgets"
      className="hidden sm:flex fixed bottom-6 right-6 z-50 gap-3"
    >
      {/* --- Bouton Langue --- */}
      <motion.div
      ...
      >
        <motion.button
          // Animation largeur lors du hover
          initial={{
            width: 48,
            height: 48,
            borderRadius: 999,
          ...
          // Couleur dynamique selon la section survolée
          className={`flex items-center ${
            overProjects ? "bg-white text-blue-600" : "bg-blue-600 text-white"
          } shadow-lg overflow-hidden relative transition-colors duration-300`}
          style={{ minWidth: 48 }}
        >
          ...
          {/* Icône globe animé */}
          <motion.span
            className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center justify-center w-12 h-12 pointer-events-none"
      ...
      >
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          // Couleur dynamique selon la section survolée
          className={`flex items-center justify-center rounded-full shadow-lg w-12 h-12 transition-colors duration-300 cursor-pointer ${
            overProjects
              ? "bg-white text-blue-600 hover:bg-white/90 hover:text-blue-700"
              : "bg-blue-600 text-white hover:bg-blue-700"
          }`}
        >
          {/* Icône soleil/lune animé */}
          <motion.div
